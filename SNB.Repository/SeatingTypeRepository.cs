...
{
    public class SeatingTypeRepository : BaseRepository<SeatingType>
    {
        private readonly SNBDbContext _context;
        public SeatingTypeRepository(SNBDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public bool IsTypeNameExist(string TypeName, string InitialTypeName)
        {
            ...
            {
                var isExist = _context.SeatingTypes.Any(x => !x.IsDeleted && x.TypeName.ToLower().Equals(TypeName.ToLower()));
